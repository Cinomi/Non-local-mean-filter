...
%REPLACE THIS
    ...
    for offsetRow=-windowR:windowR        
        for offsetCol=-windowR:windowR
            ...
            diffImg=(extendImg-movedImg).^2;
            diffIntegral=computeIntegralImage(diffImg);    % compute difference integral image with specific row and column offsets
            distances(counter)=evaluateIntegralImage(diffIntegral,movedRow,movedCol,patchSize);   % get average disstance
        end
    end
end